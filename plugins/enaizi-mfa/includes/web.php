 ...
 * Web Functions
 *  1. mfa_get_web_field
 *  2. mfa_update_web_field
 *  3. mfa_get_web_data
 *  4. mfa_update_web_data
**/
...
// 1. GET WEB FIELD
// ============================================ 
function mfa_get_web_field($item_id, $field) {
    global $wpdb;
...
// 2. UPDATE WEB FIELD
// ============================================ 
function mfa_update_web_field($item_id, $field, $value) {
    global $wpdb;
...
// 3. GET WEB DATA
// ============================================ 
function mfa_get_web_data($item_id) {
    global $wpdb;
...
// 4. UPDATE WEB DATA
// ============================================ 
function mfa_update_web_data($item_id, $data) {
    global $wpdb;
...
add_action('fluentform/before_insert_submission', function ($insertData, $data, $form) {
    ...
    // Ensure this only runs for Form ID 69
    if ((int) $form->id !== 69) { 
        return;
    }
    ...
    $website  = sanitize_text_field(\FluentForm\Framework\Helpers\ArrayHelper::get($data, 'url'));
...
// Address
add_filter('fluentform/editor_shortcode_callback_web_address', function ($value, $form) {
    $post_id = get_the_ID();
    $item_id = get_post_meta($post_id, 'item_id', true );
    $dynamicValue = mfa_get_web_field($item_id, 'address');
    return $dynamicValue;
}, 10, 2);

// Introduction
add_filter('fluentform/editor_shortcode_callback_web_introduction', function ($value, $form) {
    $post_id = get_the_ID();
    $item_id = get_post_meta($post_id, 'item_id', true );
    $dynamicValue = mfa_get_web_field($item_id, 'introduction');
    return $dynamicValue;
}, 10, 2);

// City
add_filter('fluentform/editor_shortcode_callback_web_city', function ($value, $form) {
    $post_id = get_the_ID();
    $item_id = get_post_meta($post_id, 'item_id', true );
    $dynamicValue = mfa_get_web_field($item_id, 'city');
    return $dynamicValue;
}, 10, 2);

// Country
add_filter('fluentform/editor_shortcode_callback_web_country', function ($value, $form) {
    $post_id = get_the_ID();
    $item_id = get_post_meta($post_id, 'item_id', true );
    $dynamicValue = mfa_get_web_field($item_id, 'country');
    return $dynamicValue;
}, 10, 2);

// Phone
add_filter('fluentform/editor_shortcode_callback_web_phone', function ($value, $form) {
    $post_id = get_the_ID();
    $item_id = get_post_meta($post_id, 'item_id', true );
    $dynamicValue = mfa_get_web_field($item_id, 'phone');
    return $dynamicValue;
}, 10, 2);

// Whatsapp
add_filter('fluentform/editor_shortcode_callback_web_whatsapp', function ($value, $form) {
    $post_id = get_the_ID();
    $item_id = get_post_meta($post_id, 'item_id', true );
    $dynamicValue = mfa_get_web_field($item_id, 'whatsapp');
    return $dynamicValue;
}, 10, 2);


// Website
add_filter('fluentform/editor_shortcode_callback_web_website', function ($value, $form) {
    $post_id = get_the_ID();
    $item_id = get_post_meta($post_id, 'item_id', true );
    $dynamicValue = mfa_get_web_field($item_id, 'url');
    return $dynamicValue;
}, 10, 2);

// Email
add_filter('fluentform/editor_shortcode_callback_web_email', function ($value, $form) {
    $post_id = get_the_ID();
    $item_id = get_post_meta($post_id, 'item_id', true );
    $dynamicValue = mfa_get_web_field($item_id, 'email');
    return $dynamicValue;
}, 10, 2);

// Facebook
add_filter('fluentform/editor_shortcode_callback_web_facebook', function ($value, $form) {
    $post_id = get_the_ID();
    $item_id = get_post_meta($post_id, 'item_id', true );
    $dynamicValue = mfa_get_web_field($item_id, 'fb');
    return $dynamicValue;
}, 10, 2);

// Instagram
add_filter('fluentform/editor_shortcode_callback_web_instagram', function ($value, $form) {
    $post_id = get_the_ID();
    $item_id = get_post_meta($post_id, 'item_id', true );
    $dynamicValue = mfa_get_web_field($item_id, 'insta');
    return $dynamicValue;
}, 10, 2);

// LinkedIn
add_filter('fluentform/editor_shortcode_callback_web_linkedin', function ($value, $form) {
    $post_id = get_the_ID();
    $item_id = get_post_meta($post_id, 'item_id', true );
    $dynamicValue = mfa_get_web_field($item_id, 'linkedin');
    return $dynamicValue;
}, 10, 2);

// TikTok
add_filter('fluentform/editor_shortcode_callback_web_tiktok', function ($value, $form) {
    $post_id = get_the_ID();
    $item_id = get_post_meta($post_id, 'item_id', true );
    $dynamicValue = mfa_get_web_field($item_id, 'tiktok');
    return $dynamicValue;
}, 10, 2);