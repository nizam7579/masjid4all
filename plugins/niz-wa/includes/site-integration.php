...
/**
 * Glue between Niz WA (this plugin) and the existing Enaizi User system.
 * Resolves WhatsApp senders against Enaizi's member records instead of
 * Niz WA's default "create a real WP user for every unknown number"
 * fallback, and ports Enaizi WA's action set onto Niz WA's data-driven
 * action registry (see niz_wa_seed_actions() below).
 */
...
function niz_wa_resolve_user_id( $user_id, $wa_number, $contact_name ) {
	if ( ! function_exists( 'niz_user_check' ) ) {
		return $user_id; // Enaizi User inactive — fall back to Niz WA's default resolver.
	}
	...
	return $user_id; // last resort — Niz WA's own wp_insert_user fallback.
}
...
/**
 * NOTE: Enaizi WA detects these three intents but Niz_WA_Actions never
 * defined matching methods, so confirming them today is a silent no-op.
 * These are real implementations — review the URLs/copy before go-live.
 */
